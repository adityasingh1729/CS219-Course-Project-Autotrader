...
int part3 = 0;
long long int minQuant;
int arbitFinder;
int arbitFound;
int order_num = 0;
int change = 0;
int projectedProfits = 0;
            ...
            if (subsetCardinality(subsetIndex) > 1) {
                ...
                for (int k = 0; k < checkSet.size(); k++) {
                    ...
                    for (auto stock : checkSet[k]) {
                        if (stock.first != "price" && stock.first != "quant" && stock.first != "order_num" && stock.first != "direction") {
                ...
                if (arbitFinder == 1) {
                    ...
                    if (netPrice > 0) {
                        arbitFound = 1;
                        minQuant = 10000000000000000;
                        for (auto stock : checkSet) {
                            if (stock["quant"] < minQuant) {
                                minQuant = stock["quant"];
                            }
                        }
                        // std::cout << "Arbitrage Possible, with price " << netPrice << endl;
                        arbitrage_set[subsetIndex] = netPrice;
                        arbitrage_set[-1] = minQuant;
                        // cout << "MinQuant = " << minQuant << endl;
                    }
                }
            }
        }
    ...
    for(string order : orders) {
        ...
        if (arbitFound == 1) {
            ...
            for (int i = printSet.size() - 1; i >= 0; i--) {
                ...
                if (part3 == 1) {
                    vector<string> split = split_sentence(printString, ' ');
                    split[split.size() - 2] = to_string(minQuant);
                    printString = "";
            ...
            // cout << maxPrice*minQuant << endl;
            projectedProfits += maxPrice*minQuant;
            arbitFound = 0;
